...
2.  Exercise 0.2 will teach your how to create your own branch. Each team member will create their own branch and complete the homework themselves. Note that I'll be able to see all of your commits so I'll know if you didn't do this.
...
::: {.callout-tip title="Question"}
1.  Open the terminal in VS Code (Terminal -> New Terminal). This is a bash terminal where you can run git commands.
2.  Create your own branch by typing `git checkout -b branch_name` where `branch_name` is whatever you want to name your branch. Typing `git` tells the terminal that you want to run a `git` command, `checkout` is a git command that will switch you to a different branch, the `-b` option will create a new branch for you. If you want to switch between existing branches leave out `-b`.
3.  Put your name in one of the slots below and render your document.
4.  Stage your changes by typing `git add .` or using the Source Control tab in VS Code. `git add .` will simply stage all changes. If you don't want to stage ALL of the changes you can type `git add filename` where `filename` corresponds to the files you want to stage.
...
6.  To push your changes to GitHub, type `git push`. You might need to set the upstream branch if prompted with a command like `git push --set-upstream origin branch_name`.
...
*Hint: You will need to make sure the target class is handled appropriately. If your categorical names are strings, check `model.classes_` to see the internal ordering `LogisticRegression` uses.*
:::

## Exercise 3

::: {.callout-tip title="Question"}
Create a DataFrame of integers from 0 to 50 for the number of nodes. Use the `.predict_proba()` method on your fitted model to evaluate probabilities on these integers. Extract the probabilities for the positive class (the outcome you're tracking as "dying/did not survive" or however you labeled it). 

Create a line plot with the integers from 0 to 50 on the x-axis and the predicted probabilities on the y-axis, using `matplotlib`, `seaborn`, or `plotnine`. Based on this image, estimate the input that would be needed to give an output probability of 0.75. What does this mean in the context of the model?
:::
...
-   Using a threshold value of 0.5, obtain a vector of class predictions for the test data set (e.g. evaluate `model.predict_proba(X_test)[:, 1] >= 0.5`). You need not display it.
...
::: {.callout-tip title="Question"}
Suppose you decided to create a super simple baseline model and just predict that everyone survives. What would the accuracy on the training set be? Note that you should not need to use construct a confusion matrix to answer this question.
:::
...
::: {.callout-tip title="Question"}
A threshold of 0.5 isn't necessarily the best choice for the threshold. Write out a `for` loop (or use list comprehensions/NumPy arrays) to test every threshold between 0 and 1 (increase by steps of 0.01). Calculate the accuracy, recall, and precision at each threshold. Create a single line-plot with the the threshold on the x-axis and the accuracy, recall, and precision on the y-axis. Hint: you should NOT be fitting any models here, just re-evaluating metrics using `predict_proba`.
:::
...
::: {.callout-tip title="Question"}
Produce an ROC curve and calculate the AUC statistic on the test set. You can use something like `RocCurveDisplay.from_estimator`, or calculate it manually with `roc_curve` and `roc_auc_score` from `sklearn.metrics`. Try to report your AUC. Comment on your results.
:::
...
Create the best scikit-learn model that you can. Best model gets a high five from Dr. Friedlander. Note: split your data into a train, and test sets using the random state 10520.
:::
