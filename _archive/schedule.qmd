...
d |>
  ...
  text_transform(cells_body(columns = c(`hack-a-thon`)),
    fn = function(x) {
      ifelse(is.na(x), fontawesome::fa("space-shuttle", fill_opacity = 0.1),
        glue::glue("<a href = {x}>{fontawesome::fa('space-shuttle')}</a>")
      )
    }
  ) |>
  ...
  cols_align(
    align = "center", 
    columns = c(lecture, dow, week, prepare, slides, hw, project, job, `hack-a-thon`)
    ) |>
  cols_align(
    align = "left", 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(week, topic, slides, `hack-a-thon`)
    )
  )
```
:::
