---
title: 'MATH 4025: The Big Picture'
---
...
# from ISLP import load_data # Assuming ISLP is installed or will be
```
...
# Loading AmesHousing data. 
# Note: Ensure 'pyreadr' is installed if reading RDS, or convert data to CSV/Parquet.
# Assuming CSV for now or fallback.
try:
    ames = pd.read_csv('../data/AmesHousing.csv') # Placeholder if CSV exists
except FileNotFoundError:
    # If using RDS and pyreadr is available:
    # import pyreadr
    # ames = pyreadr.read_r('../data/AmesHousing.rds')[None]
    # For display purposes in slides if data missing:
    ames = pd.DataFrame(columns=['Order', 'PID', 'MS SubClass', 'MS Zoning', 'Lot Frontage', 'Lot Area', 'Street', 'Alley', 'Lot Shape']) # Dummy
    pass

# Display head
# ames.head(10).iloc[:, :9]
# Using markdown table for display if kable was used
print(ames.head(10).iloc[:, :9].to_markdown(index=False))
```
...
print(Default.sample(n=10, random_state=1).to_markdown(index=False))
```
...
print(USArrests.head(10).to_markdown())
```
