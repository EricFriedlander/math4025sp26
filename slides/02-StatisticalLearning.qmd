...
2. What happens to `Income` as `Seniority` increases? What is the difference between the information that the three plots (combine the first two) give you in answering this questions?
...
# Assuming toy_data, a, b, and c are already defined
# Example function definition for the "true model"
def true_model_func(x):
    return a + (b * np.sqrt(x)) + (c * np.sin(x))

g3 = (
    ggplot(toy_data, mapping=aes(x='input', y='response'))
    + geom_point()
    # True Model
    + stat_function(
        aes(color="'true model'"), 
        fun=true_model_func, 
        size=1.5
    )
    # Linear Model
    ...
    # Non-linear Model (requires 'statsmodels' installed)
    + geom_smooth(
        aes(color="'non-linear model'"),
        span=0.25,
        se=False
    )
    + scale_color_manual(
...
-   Relevant questions in model selection:
    -   How many observations $n$ and variables $p$? 
